...
const int wanted_input_width = 375;
const int wanted_input_height = 1242;
const int wanted_input_channels = 3;
const float input_mean = 117.0f;
const float input_std = 1.0f;
const std::string input_layer_name = "image_input";
const std::vector<std::string> output_layer_name = {"probability/score","probability/class_idx","bbox/trimming/bbox"};
//const std::string output_layer_name = "probability/score,probability/class_idx,bbox/trimming/bbox";
//see how variables are fetched and used. If can get from one name in string to list of them. How they feed actual image to this image variable
...
- (void)runCNNOnFrame:(CVPixelBufferRef)pixelBuffer {
  ...
  if (tf_session.get()) {
    ...
    if (!run_status.ok()) {
      ...
      tensorflow::Tensor *ids =&outputs[1];
      tensorflow::Tensor *boundingBoxes =&outputs[2];
        
        std::cout << "bounding boxes here";
        std::cout << boundingBoxes->flat<float>()(0) <<" ";
        std::cout << boundingBoxes->flat<float>()(1)<<" ";
        std::cout << boundingBoxes->flat<float>()(2)<<" ";
        std::cout << boundingBoxes->flat<float>()(3)<<" ";
        std::cout << boundingBoxes->flat<float>()(4)<<" ";
        std::cout << boundingBoxes->flat<float>()(5)<<" ";
        std::cout << boundingBoxes->flat<float>()(6)<<" ";
        std::cout << boundingBoxes->flat<float>()(7)<<" ";
        std::cout << boundingBoxes->flat<float>()(8)<<" \n";
      ...
      for (int index = 0; index < predictions.size(); index += 1) {
          //std::cout << "outputs id vector here: ";
          //std::cout << ids->flat<tensorflow::int64>()(index);
          //std::cout << "bounding boxes here";
          //std::cout << boundingBoxes->flat<float>(); // need to figure out how to get 4 values for each box here
          
          tensorflow::int64 labelId = ids->flat<tensorflow::int64>()(index);
          
        const float predictionValue = predictions(index);
        if (predictionValue > 0.05f) {
          std::string label = labels[labelId];
